...
async function main() {
  ...
  targetOrders.forEach((o: any) => {
    ...
    console.log(`Phí thu khách: ${(o.shipping_fee || 0).toLocaleString()} VND`);
    console.log(`Phí trả đối tác: ${(o.partner_fee || 0).toLocaleString()} VND`);
    console.log(`Sản phẩm:`);
    if (o.items && Array.isArray(o.items)) {
      o.items.forEach((item: any) => {
        const price = item.price ? item.price.toLocaleString() : '0';
        console.log(` - ${item.name || item.product_name} | SL: ${item.quantity} | Giá: ${price}`);
      });
    } else {
