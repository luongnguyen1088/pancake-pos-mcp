...
async function main() {
  ...
  const lossOrders = allPartnerOrders
    .map(o => ({
      ...
      // Attempting to find customer name in various fields
      cName: o.customer?.name || o.customer_name || o.bill_full_name || 'Khách lẻ'
    }))
    .filter(o => o.loss < 0)
  ...
  lossOrders.slice(0, 25).forEach((o, i) => {
    console.log(`| ${i+1} | ${o.id} | ${o.inserted_at.split('T')[0]} | ${o.cName} | ${o.shipping_fee.toLocaleString()} | ${o.partner_fee.toLocaleString()} | ${o.loss.toLocaleString()} |`);
  });
